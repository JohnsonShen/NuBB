...
uint32_t PWM_ConfigOutputChannel(PWM_T *pwm, uint32_t u32ChannelNum, uint32_t u32Frequency, uint32_t u32DutyCycle)
{
    ...
    (*(volatile uint32_t *) (&((pwm)->CMPDAT0) + (u32ChannelNum)) = (u32DutyCycle * (u16CNR + 1) / 100 - 1));
    (*(volatile uint32_t *) (&((pwm)->PERIOD0) + (u32ChannelNum)) = (--u16CNR));

    (pwm)->WGCTL0 &= ~((0x3ul << (0)) << (u32ChannelNum * 2 + (16)));
    (pwm)->WGCTL0 |= ((1UL) << (u32ChannelNum * 2 + (16)));
    (pwm)->WGCTL0 &= ~((0x3ul << (16)) << (u32ChannelNum * 2 + (16)));
...
void PWM_Stop(PWM_T *pwm, uint32_t u32ChannelMask)
{
    ...
    for(i = 0; i < (6); i ++)
    {
        ...
        {
            *(volatile uint32_t *)(&((pwm)->PERIOD0) + i) = 0;
        }
    }
}
...
void PWM_EnablePDMA(PWM_T *pwm, uint32_t u32ChannelMask, uint32_t u32RisingFirst)
{
    ...
    for(i = 0; i < (6); i ++)
    {
        ...
        {
            u32IsOddCh = i % 2;
            (pwm)->PDMACTL = ((pwm)->PDMACTL & ~(((0x1ul << (4)) | (0x1ul << (3))) << ((i >> 1) * 8))) |                              (((u32IsOddCh << (4)) | (u32RisingFirst << (3)) |                                (0x3ul << (1)) | (0x1ul << (0))) << ((i >> 1) * 8));
...
void PWM_ConfigSyncPhase(PWM_T *pwm, uint32_t u32ChannelNum, uint32_t u32SyncSrc, uint32_t u32Direction, uint32_t u32StartPhase)
{
    (pwm)->SYNC = (((pwm)->SYNC & ~((0x3ful << (8)) | (0x7ul << (24)))) |                    ((u32Direction << (24) | u32SyncSrc << (8)) << (u32ChannelNum >> 1)));

    *(volatile uint32_t *)(&((pwm)->PHS0_1) + (u32ChannelNum >> 1)) = (*(volatile uint32_t *)(&((pwm)->PHS0_1) + (u32ChannelNum >> 1)) & ~(0xfffful << (0))) | u32StartPhase;
}

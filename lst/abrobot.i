...
void setupSystemClock()
{
	...
	CLK_EnableXtalRC((0x1ul << (2)) | (0x1ul << (1)));

	 
	CLK_WaitClockReady((0x1ul << (4)) | (0x1ul << (1)));
...
void setupRTC()
{
		CLK_EnableModuleClock(((((1) & 0x03) << 30)|((((1)) & 0x1f) << 0) | (((3) & 0x03) << 28)|(((1) & 0x07) << 25)|(((8) & 0x1f) << 20)| (((0x0) & 0x03) << 18)|(((0x0) & 0xff) << 10)|(((0x0) & 0x1f) << 5)));
	
		 
    sWriteRTC.u32Year       = 2015;
    sWriteRTC.u32Month      = 12;
    sWriteRTC.u32Day        = 14;
    sWriteRTC.u32DayOfWeek  = 0x1UL;
...
{
	setupSystemClock();
	setup_system_tick(10000);
	GPIO_SetMode(((GPIO_T *) ((0x40000000UL) + 0x040C0)),0x00000040, 0x1UL);
	(*((volatile uint32_t *)((((0x40000000UL) + 0x04800)+(0x40*(3))) + ((6)<<2))))=1;
	setupUART();
			...
			if (command == 'h') {
				...
				if(WaitDataReady(length)==1)
				{
						...
						if((buf[0] == 0x01)&&(buf[1] == 00)){ 
								...
								((UART_T *) (((0x40000000UL) + 0x00040000) + 0x30000))->DAT = ((GetBattery()>>8) & 0xFF);
								((UART_T *) (((0x40000000UL) + 0x00040000) + 0x30000))->DAT = (GetBattery() & 0xFF);
						}
						else if((buf[0] == 0x05)&&(buf[1] == 00)){								
								RTC_SetDate((2000+buf[2]), buf[3], buf[4], buf[5]);
                RTC_SetTime(buf[6], buf[7], buf[8], 1, 1);
						}
						else if((buf[0] == 0x06)&&(buf[1] == 00)){								
								motor_enable^=1;
						}
						UART_Write(((UART_T *) (((0x40000000UL) + 0x00040000) + 0x30000)),buf,length);
				}
				else
				{
						for(i=0;i<Serial_available();i++)
								buf[i] = Serial_read();
						UART_Write(((UART_T *) (((0x40000000UL) + 0x00040000) + 0x30000)),buf,Serial_available());
				}
		}
		else if(start == 0x7A){ 
				char buf[10],i;
				...
				if(WaitDataReady(length)==1)
				{
						for(i=0;i<length;i++)
								buf[i] = Serial_read();
						if((buf[0] == 0x01)&&(buf[1] == 00)){ 
								(*((volatile uint32_t *)((((0x40000000UL) + 0x04800)+(0x40*(2))) + ((10)<<2))))=(1-buf[2]);
						}
...
void loop()
{
	...
	nextTick = getTickCount()+2;

	CommandProcess();



	SensorsRead((1<<0)|(1<<1)|(1<<2)|(1<<3),1);
 ...
 SensorsRead((1<<4),1);
	...
	if(ChronographRead(2)>= 20) {
		SensorsDynamicCalibrate((1<<1)|(1<<2));
		ChronographSet(2);
		computeRC();
		armDetect();
	}

	if(getMagMode()||!(GetSensorCalState()&(1<<2)))
		nvtUpdateAHRS((1<<0)|(1<<1)|(1<<2));
	else
		nvtUpdateAHRS((1<<0)|(1<<1));

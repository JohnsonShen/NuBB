...
void DATA_FLASH_Write(uint32_t u32addr,uint32_t u32data)
{
	...
	for(i=0;i<512;i++)
		data_buff[i] = FMC_Read(DATA_Flash_Start_ADD+i*4+ u32addr/512*2048);
	
	FMC_Erase(DATA_Flash_Start_ADD+u32addr/512*2048);
	data_buff[u32addr%512]=u32data;
	
	for(i=0; i<512; i++) 
		FMC_Write(DATA_Flash_Start_ADD+i*4+ u32addr/512*2048, data_buff[i]);
...
void FlashInit()
{
	...
	SYS_UnlockReg();
	FMC_Open();
	SetDataFlashBase(0x20000);
	DATA_Flash_Start_ADD = FMC_ReadDataFlashBaseAddr();
	FMC_Close();
	SYS_LockReg();
#line 160 "FlashCtrl.c"
	printf("Flash initilize - [OK]\n");
}
void UpdateBoardVersion(_Bool erase)
{
	...
	if(erase) {
		DATA_FLASH_Write(12,0xffffffff);
	}
	else {
		BoardVersion = DATA_FLASH_Read(12);

    BoardVersion = 145;

		if(BoardVersion==0xFFFFFFFF) {
			DATA_FLASH_Write(12,145);
...
void UpdateFlashCal(int8_t sensorType, _Bool erase)
{
	...
	float mean[3], scale[3],matrix[10];
	...
	if(sensorType&(1<<1)) {
		...
		if(erase) {
			DATA_FLASH_Write(CalBase++,i162dw((int16_t)0x88));
			FlashState->GYRO_FLASH = 0;
			return;
		}
		else
		DATA_FLASH_Write(CalBase++,i162dw((int16_t)0x77));
		FlashState->GYRO_FLASH = 1;
	...
	if(sensorType&(1<<0)) {
		...
		if(erase) {
			DATA_FLASH_Write(CalBase++,i162dw((int16_t)0x88));
			FlashState->ACC_FLASH = 0;
			return;
		}
		else
		DATA_FLASH_Write(CalBase++,i162dw((int16_t)0x77));
		FlashState->ACC_FLASH = 1;
		DATA_FLASH_Write(CalBase++,float2dw(mean[0]));
		DATA_FLASH_Write(CalBase++,float2dw(mean[1]));
		DATA_FLASH_Write(CalBase++,float2dw(mean[2]));
		DATA_FLASH_Write(CalBase++,float2dw(scale[0]));
...
{
	uint8_t PIDBase, i=0;	
	float PID_FIELD[(9 + 9 + 3)];
	
	PIDBase=(((16 + 6 + 1) + 6 + 1) + 10 + 1 + 1);
	GetRollPID(&PID_FIELD[i]);i+=3;
	GetPitchPID(&PID_FIELD[i]);i+=3;
	GetYawPID(&PID_FIELD[i]);i+=3;
	GetRollRatePID(&PID_FIELD[i]);i+=3;
	GetPitchRatePID(&PID_FIELD[i]);i+=3;
	GetYawRatePID(&PID_FIELD[i]);i+=3;
	GetAltHoldPID(&PID_FIELD[i]);i+=1;
	if(erase) {
		DATA_FLASH_Write(PIDBase++,i162dw((int16_t)0x88));
		return;
	}
	else
	DATA_FLASH_Write(PIDBase++,i162dw((int16_t)0x77));
	for(i=0;i<(9 + 9 + 3);i++) {
...
void UpdateFlashRxAddress()
{
	...
	uint8_t RX_ADDRESS_FIELD[2*4];
	
	RxAddrBase=((((16 + 6 + 1) + 6 + 1) + 10 + 1 + 1) + (9 + 9 + 3) + 1);
	GetMatchAddress(RX_ADDRESS_FIELD);
	DATA_FLASH_Write(RxAddrBase++,i162dw((int16_t)0x77));
...
void FlashControl()
{
	...
	if(Action=='e') {
		char Part = GetChar();
		if(Part=='a') {
			UpdateFlashCal((1<<0), 1);
			if (report_format == 0) {
			printf("ACC CalData Erased.\n");
		}
		}
		else if(Part=='g') {
			UpdateFlashCal((1<<1), 1);
			if (report_format == 0) {
			printf("GYRO CalData Erased.\n");
		}
		}
		else if(Part=='m') {
			UpdateFlashCal((1<<2), 1);
			if (report_format == 0) {

...
uint16_t* rcValueSSV;
uint8_t	FlyMode = 0;
uint16_t RxChannel[6];

void SPI_Init(void)
{
	SYS_UnlockReg();

	CLK_EnableModuleClock(((((1) & 0x03) << 30)|((((12)) & 0x1f) << 0) | (((2) & 0x03) << 28)|(((3) & 0x07) << 25)|(((2) & 0x1f) << 20)| (((0x0) & 0x03) << 18)|(((0x0) & 0xff) << 10)|(((0x0) & 0x1f) << 5)));

	((SYS_T *) ((0x40000000UL) + 0x00000))->GPB_MFPL &= ~((0xful << (16)) | (0xful << (20)) | (0xful << (24)) | (0xful << (28)));
	((SYS_T *) ((0x40000000UL) + 0x00000))->GPB_MFPL |= ((2ul << (28)) | (2ul << (24)) | (2ul << (16)) | (2ul << (20)));
	((SYS_T *) ((0x40000000UL) + 0x00000))->GPD_MFPL &= ~(0xful << (12));	
	((SYS_T *) ((0x40000000UL) + 0x00000))->GPD_MFPL |= (8ul << (12));
	
	SYS_LockReg();

	SPI_Open(((SPI_T *) (((0x40000000UL) + 0x00040000) + 0x20000)), (0x0), ((0x1ul << (2))), 8, 1000000);   
}
...
void ssv_rc_update(void)
{
	...
	if(RxData.num==5) {
		if(FlyMode==1) {
			THR_SCALE = ((float)(1800-1200)/1024);
			ROLL_SCALE = 0.4f;
			PITCH_SCALE = 0.4f;
		}
		else {
			THR_SCALE = ((float)(1600-1200)/1024);
			ROLL_SCALE = 0.27f;
			PITCH_SCALE = 0.27f;
		}
		
		RxChannel[3] 	= ((RxData.BUF[0]<<2)|(RxData.BUF[4]>>6)&3);
		RxChannel[0] 	= ((RxData.BUF[1]<<2)|(RxData.BUF[4]>>4)&3);
		RxChannel[1] = ((RxData.BUF[2]<<2)|(RxData.BUF[4]>>2)&3);
		RxChannel[2] 	= ((RxData.BUF[3]<<2)| RxData.BUF[4]&3);

    rcValueSSV[1] = RxChannel[1] + 1000;
    rcValueSSV[3] = RxChannel[3] + 1000;
    rcValueSSV[2] = RxChannel[2] + 1000;
    rcValueSSV[0] = RxChannel[0] + 1000;
#line 142 "RC_ssv.c"
		RxData.num=0;
	}
}
void SetRemoteControl()
{
  char token0 = GetChar();
	if(token0=='p') { 
    rcValueSSV[1] = GetUnit32() + 1000;
  }
  else if(token0=='r') { 
    rcValueSSV[0] = GetUnit32() + 1000;
  }
  else if(token0=='t') { 
    rcValueSSV[3] = GetUnit32() + 1000;
  }
  else if(token0=='y') { 
    rcValueSSV[2] = GetUnit32() + 1000;
  }
}
...
{
	return ((readSsvRC(3) - 1200) + (1500 - 1390));
}
int16_t GetSSVAltitude()
{
	return (readSsvRC(3) - 1390);
}

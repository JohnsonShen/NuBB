...
void SetMotorCal()
{
  char token0 = GetChar();
    ...
    char token1 = GetChar();
    if(token1=='o') { //offset
      MotorCal.MotorOffset[R] = GetPIDfloat();
    }
    else if(token1=='s') { //scale
      MotorCal.MotorScale[R] = GetPIDfloat();
    }
  }
  else if(token0=='r') { //Right
    char token1 = GetChar();
    if(token1=='o') { //offset
      MotorCal.MotorOffset[L] = GetPIDfloat();
    }
    else if(token1=='s') { //scale
      MotorCal.MotorScale[L] = GetPIDfloat();
    }
  }
}
MotorCal_t* GetMotorCal()
{
  return &MotorCal;
}
#endif

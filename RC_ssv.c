...
#define PITCH_SCALE_BASIC  0.27f//0.027f
...
bool RC_SSV_Init(void)
{
	bool beConnected;
	//SPI_Init();
	beConnected = true;//IsSSVReceiverConnected();
	if(beConnected) {
	//SSV7241_Enable();	
	rcValueSSV = getValue();
  RxData.BUF[0] = 0x80;
...
#ifdef ABROBOT
    rcValueSSV[PITCH_CH] = RxChannel[PITCH_CH] + 1000;
    rcValueSSV[THR_CH] = RxChannel[THR_CH] + 1000;
    rcValueSSV[YAW_CH] = RxChannel[YAW_CH] + 1000;
    rcValueSSV[ROLL_CH] = RxChannel[ROLL_CH] + 1000;
#else
...
void SetRemoteControl()
{
  char token0 = GetChar();
	if(token0=='p') { //Pitch
    rcValueSSV[PITCH_CH] = GetUnit32() + 1000;
  }
  else if(token0=='r') { //Roll
    rcValueSSV[ROLL_CH] = GetUnit32() + 1000;
  }
  else if(token0=='t') { //Thr
    rcValueSSV[THR_CH] = GetUnit32() + 1000;
  }

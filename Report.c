...
{
	float Euler[3],Altitude,HeadHold;
	
	nvtGetEulerRPY(Euler);
  HeadHold = getHeadHold();
#if STACK_BARO
	...
	if (report_format == REPORT_FORMAT_BINARY) {
		...
		printf("@RPYA:%f,%f,%f,%f,%f\n",Euler[0],Euler[1],Euler[2],Altitude,HeadHold);
}
...
void report_actuator_status()
{
  ...
  pActuator = getActuator();
	if (report_format == REPORT_FORMAT_BINARY) {
		Serial_write((char*)pActuator, 10);
	}
	else if (report_format == REPORT_FORMAT_TEXT) {
		printf("@mActuator T,R,P,Y,S:%d,%d,%d,%d,%d\n", pActuator->actuatorThrust,pActuator->actuatorRoll, pActuator->actuatorPitch,pActuator->actuatorYaw, pActuator->actuatorSpeed);
	}
}
#endif
...
void report_status()
{
  ...
  else if (mode == 'h') // 'h'all status
		report_mode = REPORT_HALL_STATUS; 
  else if (mode == 't') // ac't'uator status
		report_mode = REPORT_ACTUATOR_STATUS; 
}
void report_sensors()
{
...
#endif
#ifdef ABROBOT
  else if (report_mode == REPORT_HALL_STATUS) {
		report_motor_speed();
  }
  else if (report_mode == REPORT_ACTUATOR_STATUS) {
		report_actuator_status();
  }
#endif
}
